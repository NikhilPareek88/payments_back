...
@RestController
@RequestMapping("/payment-card-cost")
@Validated
public class CardCostController {
    ...
    public ResponseEntity<CardCostResponse> getCardCost(@Valid @RequestBody CardRequest cardRequest) {
        try {
            logger.info("Card cost request: {}", cardRequest.cardNumber());

            // Validation already placed on CardRequest, so not required here.
            //validateRequest(cardRequest);
    ...
    private static ResponseEntity<CardCostResponse> validateRequest(CardRequest cardRequest) {
        if (cardRequest.cardNumber().length() < 8 || cardRequest.cardNumber().length() > 19) {
